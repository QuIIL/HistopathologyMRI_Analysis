...
class UNet(nn.HybridBlock):
    ...
    def __init__(self, num_ds=4, add_scale_layer='tanh', bn_mom=.9, bn_eps=1e-5, base_channel=16, backbone_name='vgg'):
        ...
        self.layer_names_us = ['layer%d_us' % i for i in range(num_ds - 1, -1, -1)]
        ...
        with self.name_scope():
            ...
            for i, layer in enumerate(self.layer_names_us):
                self.__setattr__(layer, process_layer(channels=(base_channel * (2 ** i)), bn_mom=bn_mom, bn_eps=bn_eps))
            self.downsample = nn.MaxPool2D()
            self.upsamples = []

            for i in range(self.num_ds - 1, -1, -1):
                self.__setattr__('upsample%d' % i, deconv_factory(ksz=2, stride=2, pad=0, channels=base_channel * (2 ** i), act='relu', bn=True, bn_mom=.9, bn_eps=1e-5))
    ...
    def hybrid_forward(self, F, x, *args, **kwargs):
        ...
        upsampled = self.__getattribute__('upsample%d' % (self.num_ds-1))(outs_ds['out%d' % self.num_ds])
        up_cropped_concat = F.concat(outs_ds['out%d' % (self.num_ds-1)], upsampled, dim=1)
        outs_us = {'out%d' % (self.num_ds-1):
                       self.__getattribute__(self.layer_names_us[-1])(up_cropped_concat)}

        for l in range(self.num_ds-2, -1, -1):
            upsampled = self.__getattribute__('upsample%d' % l)(outs_us['out%d' % (l+1)])
            up_cropped_concat = F.concat(outs_ds['out%d' % l], upsampled, dim=1)
            outs_us['out%d' % l] = self.__getattribute__(self.layer_names_us[l])(up_cropped_concat)
...
if __name__ == "__main__":
    ...
    net = UNet(num_ds=4, base_channel=32, backbone_name='resnet')
    ...
    print(net)
    x = nd.random.normal(0, 1, shape=(8, 2, 256, 256), ctx=ctx)
    outs_us = net(x)
    print(net.summary(x))
    print(outs_us.shape)
    ...
    # x = sym.Variable('data')
    # outs_us = net(x)
    # plot_network(outs_us, shape={'data': (2, 2, 256, 256)}, title='UNet_padding',
    #              node_attrs={"shape": "oval", "fixedsize": "false"}).view('UNet_padding')
