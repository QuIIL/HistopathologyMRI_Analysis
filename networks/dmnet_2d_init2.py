...
class Init:
    ...
    def __init__(self, units=[6, 12, 24, 48, 96], num_stage=4, reduction=.5, init_channels=8, growth_rate=4,
                 ...
                 scale_layer='tanh', num_group_norm=4):
        self.units = units
        ...
        self.num_group_norm = num_group_norm
...
class GroupNorm(HybridBlock):
    ...
    def __init__(self, eps=1e-5, multi_precision=False, num_group=0,
        ...
        with self.name_scope():
            self.weight = self.params.get('weight', grad_req='write', init=gamma_initializer, differentiable=True,
                                          ...
                                          shape=(1, in_channels, 1, 1))  # shape=(1, num_channels, 1, 1, 1))
    ...
    def infer_shape(self, *args):
        for i in self.collect_params().values():
            setattr(i, 'shape', (1, args[0].shape[1], 1, 1))
    ...
    def __init__(self, opts):
        ...
        if opts.bottle_neck:
            if opts.norm_type is 'batch':
                self.bblock.add(BatchNorm())
            elif opts.norm_type is 'group':
                self.bblock.add(GroupNorm(num_group=opts.num_group_norm))
            elif opts.norm_type is 'instance':
                self.bblock.add(InstanceNorm())
            elif opts.norm_type is 'layer':
                self.bblock.add(LayerNorm())
            if opts.activation in ['leaky']:
                self.bblock.add(LeakyReLU(alpha=opts.alpha))
            elif opts.activation in ['mish']:
                self.bblock.add(Mish())
            else:
...
class TransitionBlock(HybridBlock):
    ...
    def __init__(self, opts, num_filters, pool_type='avg'):
        ...
        if opts.norm_type is 'batch':
            self.tblock.add(BatchNorm())
        elif opts.norm_type is 'group':
            self.tblock.add(GroupNorm(num_group=opts.num_group_norm))
        elif opts.norm_type is 'instance':
            self.tblock.add(InstanceNorm())
        elif opts.norm_type is 'layer':
            self.tblock.add(LayerNorm())
        if opts.activation in ['leaky']:
            self.tblock.add(LeakyReLU(opts.alpha))
        elif opts.activation in ['mish']:
            self.tblock.add(Mish())
        else:
...
class TransitionBlock_v1(HybridBlock):
    """Return TransitionBlock Unit for building DenseNet
    Parameters
    ----------
    num_stage : int
    ...
    def __init__(self, opts, num_filters, pool_type='avg'):
        ...
        if opts.activation in ['leaky']:
            ...
            self.tblock.add(Activation(opts.activation))
        self.tblock.add(Conv2D(channels=int(num_filters * opts.reduction), kernel_size=(3, 3),
                               strides=(2, 2), use_bias=opts.use_bias, padding=(1, 1)))
        if opts.drop_out > 0:
            self.tblock.add(Dropout(opts.drop_out))
...
def conv_factory(opts, num_filters, kernel_size, stride=1, group=1):
    ...
    if opts.norm_type is 'batch':
        ...
        out.add(GroupNorm(num_group=opts.num_group_norm))
    if opts.activation == 'leaky':
        out.add(LeakyReLU(opts.alpha))
    elif opts.activation == 'mish':
        out.add(Mish())
    else:
...
class EncoderBlock_v0(HybridBlock):
    ...
    def __init__(self, opts, num_unit, num_filters, trans_block=True):
        ...
        if trans_block:
            ...
            self.eblock.add(MaxPool2D(pool_size=(3, 3),
...
class DecoderBlock(HybridBlock):
    ...
    def __init__(self, opts, num_filters, res_block=True, factor=1, group=1):
        ...
        if opts.norm_type is 'batch':
            self.dcblock.add(BatchNorm())
        elif opts.norm_type is 'instance':
            self.dcblock.add(InstanceNorm())
        elif opts.norm_type is 'layer':
            self.dcblock.add(LayerNorm())
        elif opts.norm_type is 'group':
            self.dcblock.add(GroupNorm(num_group=opts.num_group_norm))
        if opts.activation in ['leaky']:
            self.dcblock.add(LeakyReLU(opts.alpha))
        elif opts.activation in ['mish']:
            self.dcblock.add(Mish())
        else:
    ...
    def __init__(self, opts):
        ...
        with self.net.name_scope():
            ...
            if opts.norm_type is 'batch':
                ...
                self.net.add(LayerNorm())
            elif opts.norm_type is 'group':
                self.net.add(GroupNorm(num_group=opts.num_group_norm))
            if opts.activation in ['leaky']:
            ...
            self.net.add(Conv2D(kernel_size=(1, 1), channels=2, use_bias=opts.use_bias))
            # if opts.norm_type is 'batch':
            #     self.net.add(BatchNorm())
            # elif opts.norm_type is 'instance':
            #     self.net.add(InstanceNorm())
