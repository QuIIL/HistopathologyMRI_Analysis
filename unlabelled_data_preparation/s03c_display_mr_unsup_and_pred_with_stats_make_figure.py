...
def show_targets_separate(img, _all_sli, _labels, img_name, figsize_unit=2, suffix='', tissue_type='EPI',
                          density_range=(0, 1)):
    """"""
    ...
    for sl in range(img.shape[1]):
        ...
        if 'benign' in _labels[sl]:
            group = groups[1]
        elif '3+' in _labels[sl]:
            group = groups[2]
        else:
            group = groups[3]

        dir_fig = 'MRIs/targets_with_DensityPrediction_withStats' + suffix + '/' + group
        pred_name = f'{img_name}_{sl}_{tissue_type}_{mean_density[sl]:.3f}_{_labels[sl]}'
        if tissue_type == 'EPI':
            plt.imsave(f'{dir_fig}/{img_name}_{sl}_T2_{mean_T2[sl]:.0f}_{_labels[sl]}.png', draw_contour(t2_fig[sl], contour), cmap='gray', vmax=1)
            plt.imsave(f'{dir_fig}/{img_name}_{sl}_ADC_{mean_ADC[sl]:.0f}_{_labels[sl]}.png', draw_contour(adc_fig[sl], contour), cmap='gray', vmax=1)
...
def get_mean_data(d):
    ...
    def process(sub_id):
        nonlocal intensity_T2_collection, intensity_ADC_collection
        ...
        for m_idx in range(mr[1].shape[1]):
            ...
            densities, intensity_T2, intensity_ADC = show_targets_separate(_img, all_sli, all_labels, img_name=sub_id,
                                                                           suffix=suffix,
                                                                           tissue_type=tissue_types[m_idx],
