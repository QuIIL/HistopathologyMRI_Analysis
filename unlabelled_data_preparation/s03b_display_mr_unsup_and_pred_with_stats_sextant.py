...
def show_targets(img, _all_sli, img_name, figsize_unit=2, suffix='', tissue_type='EPI'):
    """"""
    img[-2] = img[-2] * img[-1]
    ...
    #           fontdict=font) for i in range(len(_all_sli))]
    #
    # dir_fig = 'MRIs/targets_sextant_with_%sDensityPrediction_withStats' % (tissue_type,) + suffix
    # mkdir(dir_fig)
...
def get_mean_data(d):
    ...
    for i_sub, sub_id in enumerate(all_ids):
        ...
        for m_idx in range(mr[1].shape[1]):
            ...
            densities, intensity_T2, intensity_ADC = show_targets(_img, all_sli, img_name=sub_id,
...
if __name__ == "__main__":
    ...
    # for tissue_type in ['EPI', 'NUC', 'STR']:
    ...
    futures = []
    # with ThreadPoolExecutor(len(diameters)) as pool:
    with ThreadPoolExecutor(len(diameters)) as pool:
    ...
    df.to_excel('spread_sheets/RadPath_Mean_Values_Sextant.xlsx', sheet_name='Sextant')
