...
def show_targets(img, _all_sli, _labels, _all_ROIs, img_name, figsize_unit=2, suffix='', tissue_type='EPI', _no_figs=False):
    """"""
    img[-2] = img[-2] * img[-1]
    ...
    [plt.text(img[0].shape[1] * i + 10, 30, '%s' % _all_ROIs[i],
              fontdict=font_ROI) for i in range(len(_all_ROIs))]
...
def get_mean_data(d, no_figs):
    ...
    with open('data/subject_list_extra_dir.txt', 'rb') as fp:
        subject_dirs = pickle.load(fp)
    subject_dirs_dict = {sd.split('\\')[-1]: sd for sd in subject_dirs}
    f = pd.read_excel('Extra_Path_Report.xlsx', 'FinalReport')  # FilteredReport2
    ...
    for i_sub, sub_id in enumerate(all_ids):
        ...
        for m_idx in range(mr[1].shape[1]):
            ...
            densities, intensity_T2, intensity_ADC = show_targets(_img, all_sli, all_labels, all_ROIs, img_name=sub_id,
                                                                  suffix=f'_{d}_Extra', tissue_type=tissue_types[m_idx],
                                                                  _no_figs=no_figs)
...
if __name__ == "__main__":
    ...
    dir_inputs = r'MRIs/inputs_extra/'
    ...
    no_figs = False
    max_workers = len(diameters) if no_figs else 1
    futures = []
    with ThreadPoolExecutor(max_workers) as pool:
        for diameter in diameters:  # point
            futures.append(pool.submit(get_mean_data, diameter, no_figs))
    ...
    with pd.ExcelWriter('spread_sheets/RadPath_Mean_Values_Extra.xlsx', engine="openpyxl", mode="a") as writer:
        df.to_excel(writer, sheet_name='Extra')
